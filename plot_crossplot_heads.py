...
sim_MAN_2020 = pd.read_csv(f'../../hpham/100BC_Calibration_model/PEST_2012-2020/bore_sample_output_manual.dat',names = ["WELL_NAME", "Date","Time", "Head (m)"],delimiter="   ")
...
#%% Colored plot to reproduce MPR figure AWLN 2014
plt.rc('xtick', labelsize=18)
plt.rc('ytick', labelsize=18)
sim_AWLN_2014 = pd.read_csv(f'../../final_deliverables/Head_AWLN/bore_sample_output_AWLN_2014.dat',names = ["WELL_NAME", "Date","Time", "Head (m)"],delimiter="   ")
obs_AWLN_2014 = pd.read_csv(f'../../final_deliverables/Head_AWLN/Bore_Sample_File_in_model_AWLN_2014.csv',names = ["WELL_NAME", "Date","Time", "Head (m)"],delimiter=",")
...
obs_AWLN_2014['color'] = obs_AWLN_2014['WELL_NAME'].map(color)
sim_AWLN_2014['color'] = sim_AWLN_2014['WELL_NAME'].map(color)

fig, ax = plt.subplots(nrows=1, ncols = 1, figsize=(10,10))  #15,10
wells = obs_AWLN_2014['WELL_NAME'].unique().tolist()
for well in wells:
    df = obs_AWLN_2014[obs_AWLN_2014['WELL_NAME'] == well]
    df2 = sim_AWLN_2014[sim_AWLN_2014['WELL_NAME'] == well]
    ax.scatter(df['Head (m)'],df2['Head (m)'] , c=df2['color'], s=50, alpha = 0.9, label = str(well)[4:])
    ...
    ax.set_xlabel('Observed Head 2012-2014', fontweight='bold', fontsize=18)
    ax.grid(which='both', alpha = 0.5)
# plt.savefig('output/crossplots/awln-2014-v2.png', bbox_inches='tight') 
plt.show()

#%% Colored plot to reproduce MPR figure AWLN 2020
plt.rc('xtick', labelsize=18)
plt.rc('ytick', labelsize=18)
sim_AWLN_2020 = pd.read_csv(f'../../final_deliverables/Head_AWLN/bore_sample_output_AWLN.dat',names = ["WELL_NAME", "Date","Time", "Head (m)"],delimiter="   ")
obs_AWLN_2020 = pd.read_csv(f'../../final_deliverables/Head_AWLN/Bore_Sample_File_in_model_AWLN.csv',names = ["WELL_NAME", "Date","Time", "Head (m)"],delimiter=",")
...
for well in wells:
    ...
    ax.set_xlabel('Observed Head 2012-2020', fontweight='bold', fontsize=18)
    ax.grid(which='both', alpha = 0.5)
# plt.savefig('output/crossplots/awln-2020-v3.png', bbox_inches='tight') 
plt.show()

#%% Colored plot AWLN 2020 indiivudal plots - three rows
plt.rc('xtick', labelsize=18)
plt.rc('ytick', labelsize=18)
...
#%% Compute mean error, RMSE, mean absolute error

...
def func_dirn_targets(ipst_2012_2014, pst_obs_cols, nsp):  # HP
    """
...
def Head_MAN_v1(var):
    ...
    df2 = pd.DataFrame()
    df2['Well Name'] = df.agg(lambda x: f"MAN{x['num']}", axis=1)
    df2['Val'] = df['head'].round(3)
    df2['Weight'] = 0
    df2['Group'] = 'Head_MAN'

    # Assign weight
    df2['Weight'][df['num'] <= cutoff_man_obs] = 1
    df2['Weight'][df2['Well Name'] == 'MAN79'] = 0.01
    # write to file
    # df2.to_csv('output/pst_main/head_MAN4pst.csv', index=False, sep='\t')
    # print(f'Nobs = {df2.shape[0]}\n')
...
def magn_AWLN(cutoff_magn_AWLN):  # MP script v0
    # Define some input files/parameters
...
def dirn_AWLN(cutoff_magn_AWLN):  # MP script v0
    # Define some input files/parameters
    ifile = f'../../final_deliverables/preprocess/Truex_well_network_2020/TruexWells_all_data_2012-2020_magdir.csv'
...
def dirn_AWLN_v1(cutoff_truex, ipst_2012_2014):  # MP script
    # Define some input files/parameters
    ifile = f'../../final_deliverables/preprocess/Truex_well_network_2020/TruexWells_all_data_2012-2020_magdir.csv'
    ofile_ins = 'output/pst_ins/dirn_AWLN.ins'
    # Delete the old file if existing
    ...
    # Assign weight
    # df2['Weight'][df['num'] <= cutoff_truex] = 0.06
    # special case: not constant, must import weight from pst file
...
def dirn3_SRC_v1(cutoff_truex):  # MP's script
    # Define some input files/parameters
    ifile = f'../../final_deliverables/sdirn/Bore_Sample_File_in_model_100C7_1_dirn3.csv'
    ofile_ins = 'output/pst_ins/dirn_src_100C7_1_3.ins'
    # Delete the old file if existing
    ...
    # Assign weight
    df2['Weight'][df['num'] <= cutoff_truex] = 0.04

    n_dirn3 = 285  # extended model overlapping SPs from truex network
    # cut values at 102 until you hear back from Helal.
    df3 = df2.iloc[0:n_dirn3]
    # write to file
    # df3.to_csv('output/pst_main/dirn3_src4pst.csv', index=False, sep='\t')
    # print(f'Nobs = {df2.shape[0]}\n')
...
def dirn_AWLN_MP2(cutoff_truex, ipst_2012_2014):  # 11/14/2020
    # Define some input files/parameters
    ifile = f'../../final_deliverables/preprocess/Truex_well_network_2020/TruexWells_all_data_2012-2020_magdir.csv'
    ofile_ins = 'output/pst_ins/dirn_AWLN.ins'
    # Delete the old file if existing
...
def dirn3_SRC_MP2(cutoff_truex):  # MP 11/14/2020
    # Define some input files/parameters
    ifile = f'../../final_deliverables/sdirn/Bore_Sample_File_in_model_100C7_1_dirn3.csv'
...
def func_dirn_targets_MP2(ipst_2012_2014, pst_obs_cols, nsp):  # MP 11/14/2020
    """
    Get pst block and ins files for dirn targets
    """
...
if __name__ == "__main__":
    ...
    df_Head_MAN = Head_MAN_v2()

    # MP [4] magn_AWLN and dirns_AWLN
    cutoff_truex = 102  # length of extended model: 285
    df_magn_AWLN = magn_AWLN(cutoff_truex)
    # df_dirn_AWLN = dirn_AWLN(cutoff_truex) # an older version
    df_dirn_AWLN = dirn_AWLN_MP2(cutoff_truex, ipst_2012_2014)

    # MP [5]
    # using cutoff_truex for truex network number of overlapping SPs.
    # actual length of dirn3: 436, extended model: 1306
    # df_dirn3_SRC = dirn3_SRC(cutoff_truex)  # an older version
    df_dirn3_SRC = dirn3_SRC_MP2(cutoff_truex)

    # [3] Delta_4-14, 'Delta_5-8', 'Delta_8-6',
    df_delta = Delta_targets(cutoff_sp)

    # [12] dirn_* targets
    # df_dirn = func_dirn_targets(ipst_2012_2014, pst_obs_cols, nsp) # old
    df_dirn = func_dirn_targets_MP2(ipst_2012_2014, pst_obs_cols, nsp)
